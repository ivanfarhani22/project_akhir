...
<div class="max-w-6xl mx-auto">
    <div class="flex items-start justify-between gap-4 mb-6">
        <div class="min-w-0">
            <div class="text-[11px] text-slate-500">Admin / Quiz</div>
                ...
                <div class="p-5">
                    <form method="POST" action="{{ route('admin.assignments.quiz.upsert', $assignment) }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" min="1" name="time_limit_minutes" value="{{ old('time_limit_minutes', $quiz->time_limit_minutes) }}"
                        ...
                        <div>
                            ...
                            <input type="number" min="1" name="attempts_allowed" value="{{ old('attempts_allowed', $quiz->attempts_allowed) }}" required
                                   class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C41E3A]/25 focus:border-[#C41E3A]">
                        </div>

                        <div>
                            <label class="block text-[12.5px] font-semibold text-slate-700 mb-1">Status</label>
                            <select name="status" required class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C41E3A]/25 focus:border-[#C41E3A]">
                        ...
                        <button type="submit"
                                class="w-full inline-flex items-center justify-center px-4 py-2 rounded-lg bg-[#C41E3A] text-white text-[13px] font-bold hover:bg-[#9B1630]">
                            Simpan
                        </button>
                    </form>
                </div>
        ...
        <div class="lg:col-span-2 space-y-5">
            <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-200">
                    ...
                    <div class="text-[12px] text-slate-500 mt-0.5">Buat soal singkat dan jelas.</div>
                </div>
                <div class="p-5">
                    <form method="POST" action="{{ route('admin.quizzes.questions.store', $assignment) }}" class="space-y-4">
                        @csrf

                        <div>
                            <label class="block text-[12.5px] font-semibold text-slate-700 mb-1">Pertanyaan</label>
                            <textarea name="question" rows="3" required
                                      class="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C41E3A]/25 focus:border-[#C41E3A]"></textarea>
                        </div>
                        ...
                        <div>
                            <label class="block text-[12.5px] font-semibold text-slate-700 mb-1">Opsi (khusus PG, 1 baris 1 opsi)</label>
                            <textarea name="options_text" rows="4" class="w-full px-3 py-2 border border-slate-300 rounded-lg" placeholder="A\nB\nC\nD"></textarea>
                            <div class="text-[11.5px] text-slate-500 mt-1">Kosongkan jika tipe bukan Pilihan Ganda.</div>
                        </div>

                        <button type="submit" class="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-slate-900 text-white text-[13px] font-bold hover:bg-black">
                            Tambah Soal
                        </button>
                    </form>
                </div>
            ...
            <div class="bg-white border border-slate-200 rounded-xl overflow-hidden">
                <div class="px-5 py-4 border-b border-slate-200 flex items-center justify-between">
                    ...
                    <span class="text-[12px] font-bold px-2.5 py-1 rounded-full bg-slate-100 text-slate-700 border border-slate-200">
                ...
                <div class="p-5 space-y-3">
                    ...
                    @else
                        @foreach($quiz->questions as $q)
                            @php $optionsText = is_array($q->options) ? implode("\n", $q->options) : ''; @endphp
                            <div class="border border-slate-200 rounded-lg p-4">
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <form method="POST" action="{{ route('admin.quizzes.questions.destroy', [$assignment, $q]) }}" onsubmit="return confirm('Hapus soal ini?')">
                                        @csrf
                                        @method('DELETE')
                                ...
                                <details class="mt-3">
                                    ...
                                    <form method="POST" action="{{ route('admin.quizzes.questions.update', [$assignment, $q]) }}" class="mt-3 space-y-3">
                                        @csrf
                                        @method('PUT')
                                        ...
                                        <button type="submit" class="inline-flex items-center justify-center px-4 py-2 rounded-lg bg-[#C41E3A] text-white text-[13px] font-bold hover:bg-[#9B1630]">
                                            Simpan Perubahan
                                        </button>
                                    </form>
                                </details>
