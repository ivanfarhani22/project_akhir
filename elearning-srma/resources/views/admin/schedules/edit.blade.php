...
<div class="max-w-4xl mx-auto px-4 py-8">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <form action="{{ route('admin.schedules.update', [$class, $schedule]) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')

            @php
                $entryType = old('entry_type');
                if (! $entryType) {
                    $entryType = $schedule->class_subject_id ? 'mapel' : 'custom';
                }
            @endphp
            ...
            <div>
                ...
                <select name="entry_type" id="entry_type" required
                    ...
                    <option value="mapel" @selected($entryType === 'mapel')>Mata Pelajaran (Presensi via Mapel)</option>
                    <option value="custom" @selected($entryType === 'custom')>Custom (Non Mapel / Kegiatan)</option>
                </select>
                @error('entry_type')
            ...
            <div id="section-mapel">
                ...
                <select name="class_subject_id" id="class_subject_id" class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg text-sm focus:outline-none focus:border-red-500 transition @error('class_subject_id') border-red-500 @enderror">
                    ...
                    @foreach($classSubjects as $cs)
                        <option value="{{ $cs->id }}" @selected(old('class_subject_id', $schedule->class_subject_id) == $cs->id)>
            ...
            <div id="section-custom" style="display:none;">
                ...
                <input type="text" name="custom_title" id="custom_title" value="{{ old('custom_title', $schedule->custom_title) }}"
            ...
            <div>
                ...
                <select name="day_of_week" id="day_of_week" required class="w-full px-4 py-2 border-2 border-gray-300 rounded-lg text-sm focus:outline-none focus:border-red-500 transition @error('day_of_week') border-red-500 @enderror">
                    ...
                    <option value="wednesday" @selected($schedule->day_of_week == 'wednesday')>Rabu (Wednesday)</option>
                    <option value="thursday" @selected($schedule->day_of_week == 'thursday')>Kamis (Thursday)</option>
                    <option value="friday" @selected($schedule->day_of_week == 'friday')>Jumat (Friday)</option>
                    <option value="saturday" @selected($schedule->day_of_week == 'saturday')>Sabtu (Saturday)</option>
