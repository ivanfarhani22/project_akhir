    ...
    <div class="stats-grid">
        <div class="stat-card classes">
            <div class="stat-info">
                ...
                <div class="stat-number">{{ $totalClassSubjects }}</div>
            </div>
    ...
    @if($classSubjects->isEmpty())
        <div class="card">
            <div class="card-body" style="text-align: center; padding: 60px 20px;">
                ...
                <p style="color: #999; font-size: 16px;">Belum ada mata pelajaran yang ditugaskan</p>
            </div>
        </div>
    @else
                    ...
                    <div class="card-body" style="padding: 20px;">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 15px;">
                            <div>
                                ...
                                <p style="font-size: 13px; color: #999;">{{ $cs->subject->name }}</p>
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 1px solid var(--border);">
                            <div style="text-align: center;">
                                ...
                                <div style="font-size: 20px; font-weight: 700; color: #0066cc;">{{ $cs->eClass->students->count() }}</div>
                            </div>
                            <div style="text-align: center;">
                                <div style="font-size: 11px; text-transform: uppercase; color: #999; font-weight: 600;">Materi</div>
                                <div style="font-size: 20px; font-weight: 700; color: #28a745;">{{ $cs->eClass->materials->count() }}</div>
                            </div>
                        </div>

                        <div style="display: flex; gap: 8px;">
                            <a href="{{ route('guru.materials.index', ['class' => $cs->eClass->id]) }}" class="btn btn-sm" style="flex: 1; background: #28a745; color: white; text-decoration: none; text-align: center;">
                                <i class="fas fa-book"></i> Materi
                            </a>
                            <a href="{{ route('guru.assignments.index', ['class' => $cs->eClass->id]) }}" class="btn btn-sm" style="flex: 1; background: #0066cc; color: white; text-decoration: none; text-align: center;">
