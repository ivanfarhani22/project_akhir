    ...
    <div class="grid grid-cols-1 sm:flex-row sm:grid-cols-2 lg:grid-cols-3 gap-5">
        @foreach($classSubjects as $cs)
            <div class="group bg-white rounded-2xl border-2 border-gray-100 hover:border-[#A41E35] hover:shadow-lg transition-all duration-200 overflow-hidden">
                ...
                <div class="p-5">
                    <h3 class="text-base font-bold text-gray-900 mb-1">{{ $cs->eClass->name }}</h3>
                    <p class="text-xs text-gray-500 mb-0.5">
                        <i class="fas fa-book-open mr-1"></i>
                        {{ $cs->subject->name }}
                    </p>
                    <p class="text-xs text-gray-400 mb-5"><i class="fas fa-users mr-1"></i> {{ $cs->eClass->students->count() }} siswa</p>

                    <a href="{{ route('guru.materials.index', ['class_subject_id' => $cs->id]) }}"
