        ...
        <div class="lg:col-span-4 bg-white border border-gray-200 rounded-xl p-4">
            <div class="flex items-center justify-between gap-3 mb-3">
                <div>
                    ...
                    <div class="text-[11px] text-gray-500 mt-0.5">Tip: gunakan <span class="font-semibold">Generate Otomatis</span> untuk rekomendasi anti bentrok.</div>
                </div>
        ...
        <div class="lg:col-span-8 bg-white border border-gray-200 rounded-xl p-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                ...
                <div class="flex items-center gap-3">
                    <button type="button"
                            @click="autoGenerate()"
                            class="inline-flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-lg font-semibold text-xs hover:bg-emerald-700 disabled:opacity-50"
                            :disabled="selectedIds.length === 0 || aiLoading">
                        <span x-show="!aiLoading">Generate Otomatis (AI)</span>
        ...
        async autoGenerate() {
            ...
            try {
                const res = await fetch(<?php echo json_encode($autoGenerateUrl, 15, 512) ?>, {
                    ...
                    body: JSON.stringify({
                        class_subject_ids: this.selectedIds.map(x => Number(x)),
                ...
                if (!res.ok) {
                    const msg = data?.message || 'Gagal generate jadwal.';
                    alert(msg);
                    return;
                }

                // Merge generated items into rows (replace rows for selected ids)
                const items = Array.isArray(data.items) ? data.items : [];

                // Ensure selection matches returned
                const ids = new Set(this.selectedIds.map(x => Number(x)));
                ...
                // Keep selectedIds unique
                this.selectedIds = Array.from(new Set(this.selectedIds.map(String)));

                if (data.unscheduled_count && Number(data.unscheduled_count) > 0) {
                    alert('Sebagian mapel belum dapat dijadwalkan karena constraint bentrok. Silakan atur manual / ubah jam operasional.');
                }
            } catch (e) {
                alert('Gagal generate jadwal (network/server).');
            } finally {
