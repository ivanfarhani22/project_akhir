...
<?php $__env->startSection('title', 'Detail Presensi - ' . $session->classSubject->eClass->name . ' - ' . $session->classSubject->subject->name); ?>
...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-extrabold text-gray-900"><?php echo e($session->classSubject->eClass->name); ?> — <?php echo e($session->classSubject->subject->name); ?></h1>
</div>
...
<div class="bg-white rounded-2xl border-2 border-gray-100 overflow-hidden mb-6">
    ...
    <div class="p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100">
        <div>
            <p class="font-bold text-gray-900"><i class="fas fa-calendar mr-2 text-gray-400"></i><?php echo e($session->attendance_date->format('l, d F Y')); ?></p>
            <p class="text-xs text-gray-400 mt-1"><i class="fas fa-clock mr-2"></i>Dibuka: <?php echo e($session->opened_at); ?></p>
        </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-100">
        <div class="text-center py-4 px-3">
            <p class="text-2xl font-extrabold text-gray-900"><?php echo e($session->records->count()); ?></p>
            <p class="text-xs text-gray-400 mt-1">Total Siswa</p>
        </div>
        <div class="text-center py-4 px-3">
            <p class="text-2xl font-extrabold text-emerald-600"><?php echo e($session->records->where('status','present')->count()); ?></p>
            <p class="text-xs text-gray-400 mt-1">Hadir</p>
        </div>
        <div class="text-center py-4 px-3">
            <p class="text-2xl font-extrabold text-red-500"><?php echo e($session->records->where('status','absent')->count()); ?></p>
            <p class="text-xs text-gray-400 mt-1">Tidak Hadir</p>
        </div>
        <div class="text-center py-4 px-3">
            <p class="text-2xl font-extrabold text-blue-600"><?php echo e($session->getAttendancePercentage()); ?>%</p>
...
<?php if($session->isOpen()): ?>
    <div class="flex flex-col sm:flex-row gap-3 mb-6">
        <form action="<?php echo e(route('guru.attendance.close', $session)); ?>" method="POST" class="attendance-form flex-1">
            <?php echo csrf_field(); ?>
        ...
        <form action="<?php echo e(route('guru.attendance.cancel', $session)); ?>" method="POST" class="attendance-form flex-1">
            <?php echo csrf_field(); ?>
...
<div class="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
    ...
    <?php if($session->records->count() > 0): ?>
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php $__currentLoopData = $session->records->sortBy('student.name'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $record): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <?php
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-5 py-3.5 text-gray-400 text-xs"><?php echo e($loop->iteration); ?></td>
                            <td class="px-5 py-3.5 font-semibold text-gray-800"><?php echo e($record->student->name); ?></td>
                            <td class="px-5 py-3.5">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border <?php echo e($sc); ?>">
                                    <i class="fas <?php echo e($ic); ?> text-[10px]"></i> <?php echo e($sl); ?>

                                </span>
                            </td>
                            <td class="px-5 py-3.5 text-gray-500 text-xs">
                                <?php echo e($record->checked_in_at ? $record->checked_in_at->format('H:i') : '—'); ?>
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('layouts.guru', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\xampp\htdocs\Project Akhir\elearning-srma\resources\views/guru/attendance/show.blade.php ENDPATH**/ ?>