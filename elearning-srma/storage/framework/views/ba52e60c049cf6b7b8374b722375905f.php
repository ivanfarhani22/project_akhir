...
<div class="max-w-7xl mx-auto px-3 sm:px-4 py-6 sm:py-8">
    ...
    <nav class="flex items-center space-x-2 mb-8 text-xs sm:text-sm text-gray-600 flex-wrap">
        ...
        <span class="text-gray-400">/</span>
        <span class="text-red-600 font-semibold truncate"><?php echo e($class->name); ?></span>
    </nav>
    ...
    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-8 gap-4">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 flex items-center gap-3 flex-wrap">
                ...
                <span class="break-words"><?php echo e($class->name); ?></span>
            </h1>
            <p class="text-gray-600 mt-2 text-xs sm:text-sm">Kelola mata pelajaran, guru, dan siswa dalam kelas</p>
        </div>
        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
            <a href="<?php echo e(route('admin.classes.edit', $class)); ?>" class="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 bg-gray-500 text-white px-3 sm:px-6 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-gray-600 transition whitespace-nowrap">
                <i class="fas fa-edit"></i> <span class="hidden sm:inline">Edit Kelas</span><span class="sm:hidden">Edit</span>
            </a>
            <form method="POST" action="<?php echo e(route('admin.classes.destroy', $class)); ?>" class="flex-1 sm:flex-none delete-form">
                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                <button type="button" onclick="confirmDelete(event, '<?php echo e($class->name); ?>')" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-red-600 text-white px-3 sm:px-6 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-red-700 transition whitespace-nowrap">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-6">
            ...
            <p class="text-gray-600 text-xs sm:text-sm"><?php echo e($class->description ?? '—'); ?></p>
        </div>
            ...
            <div class="space-y-2 text-xs sm:text-sm">
                <p>
                    ...
                    <span class="text-gray-600">
                        <?php echo e($dayTranslate[strtolower(now()->locale('id')->englishDayOfWeek)] ?? now()->locale('id')->translatedFormat('l')); ?>
                ...
                <p>
                    ...
                    <span class="text-gray-600" id="liveClock"><?php echo e(now()->format('H:i:s')); ?></span>
                </p>
                <p>
                    <span class="font-semibold text-gray-700">Ruangan:</span>
                    <span class="text-gray-600"><?php echo e($class->name ?: '—'); ?></span>
                </p>
            </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 sm:p-6">
            ...
            <div class="space-y-3 text-xs sm:text-sm">
                <div>
                    ...
                    <p class="text-xl sm:text-2xl font-bold text-red-600"><?php echo e($class->classSubjects->count()); ?></p>
                </div>
                <div>
                    <p class="text-gray-600 font-semibold mb-1">👥 Siswa</p>
                    <p class="text-xl sm:text-2xl font-bold text-red-600"><?php echo e($class->students->count()); ?></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-8">
        <div class="bg-gradient-to-r from-red-500 to-red-600 px-3 sm:px-6 py-3 sm:py-4 text-white flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <h3 class="text-base sm:text-lg font-bold flex items-center gap-2 m-0 flex-wrap">
                ...
                <span class="bg-red-700 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm"><?php echo e($class->classSubjects->count()); ?></span>
            </h3>
            <a href="<?php echo e(route('admin.class-subjects.create', $class)); ?>" class="inline-flex items-center justify-center gap-2 bg-white text-red-600 px-3 sm:px-4 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-red-50 transition w-full sm:w-auto whitespace-nowrap">
                <i class="fas fa-plus"></i> <span class="hidden sm:inline">Tambah</span><span class="sm:hidden">+</span>
            </a>
        </div>

        <div class="p-3 sm:p-6">
            <?php if($class->classSubjects->isEmpty()): ?>
                <div class="text-center py-8 sm:py-12">
                    ...
                    <a href="<?php echo e(route('admin.class-subjects.create', $class)); ?>" class="inline-flex items-center justify-center gap-2 bg-red-500 text-white px-4 sm:px-6 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-red-600 transition whitespace-nowrap">
                    ...
                    <?php $__currentLoopData = $class->classSubjects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="bg-gray-50 border-2 border-gray-200 rounded-lg p-3 sm:p-5 hover:shadow-md transition">
                            <div class="pb-3 sm:pb-4 border-b-2 border-gray-200 mb-3 sm:mb-4">
                                <h4 class="text-sm sm:text-base font-bold text-gray-900 m-0 mb-1">📖 <?php echo e($cs->subject->name); ?></h4>
                                <p class="text-xs text-gray-600 m-0">Kode: <?php echo e($cs->subject->code); ?></p>
                            </div>
                            <div class="mb-3 sm:mb-4">
                                <p class="text-xs text-gray-600 font-semibold mb-1">Guru Pengajar</p>
                                <p class="text-xs sm:text-sm font-semibold text-gray-900 m-0 truncate">👤 <?php echo e($cs->teacher->name); ?></p>
                            </div>
                            <?php if($cs->description): ?>
                                <p class="text-xs sm:text-sm text-gray-600 mb-3 sm:mb-4 line-clamp-2"><?php echo e(Str::limit($cs->description, 60)); ?></p>
                            <?php endif; ?>
                            <div class="flex flex-col sm:flex-row gap-2 pt-3 sm:pt-4 border-t-2 border-gray-200">
                                <a href="<?php echo e(route('admin.class-subjects.edit', [$class, $cs])); ?>"
                                ...
                                <form method="POST"
                                      action="<?php echo e(route('admin.class-subjects.destroy', [$class, $cs])); ?>"
                                      class="flex-1 delete-form">
                                    <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                    <button type="button" onclick="confirmDelete(event, '<?php echo e($cs->name); ?>')" class="w-full inline-flex items-center justify-center gap-2 bg-red-600 text-white px-3 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-red-700 transition whitespace-nowrap">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden mb-8">
        <div class="bg-gray-50 px-3 sm:px-6 py-3 sm:py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <h3 class="text-base sm:text-lg font-bold text-gray-900 m-0 flex items-center gap-2 flex-wrap">
                ...
                <span class="bg-yellow-100 text-yellow-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-semibold"><?php echo e($class->schedules->count()); ?></span>
            </h3>

            <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
                <a href="<?php echo e(route('admin.schedules.bulk.edit', $class)); ?>" class="inline-flex items-center justify-center gap-2 bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-blue-700 transition w-full sm:w-auto whitespace-nowrap">
                    <i class="fas fa-wand-magic-sparkles"></i> <span class="hidden sm:inline">Bulk Scheduling</span><span class="sm:hidden">Bulk</span>
                </a>

                <a href="<?php echo e(route('admin.schedules.create', $class)); ?>" class="inline-flex items-center justify-center gap-2 bg-yellow-500 text-white px-3 sm:px-4 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-yellow-600 transition w-full sm:w-auto whitespace-nowrap">
        ...
        <div class="p-3 sm:p-6">
            <?php if($class->schedules->isEmpty()): ?>
                <div class="text-center py-8 sm:py-12">
            ...
            <?php else: ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-xs sm:text-sm">
                        ...
                        <tbody>
                            <?php $__currentLoopData = $class->schedules; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $schedule): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 font-semibold text-gray-900"><?php echo e($dayTranslate[$schedule->day_of_week] ?? $schedule->day_of_week); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-gray-700 hidden sm:table-cell"><strong><?php echo e($schedule->start_time); ?> – <?php echo e($schedule->end_time); ?></strong></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-gray-700 truncate"><?php echo e($schedule->classSubject->subject->name); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-gray-700 hidden md:table-cell truncate"><?php echo e($schedule->classSubject->teacher->name); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-gray-700 hidden lg:table-cell"><?php echo e($schedule->room ?? '—'); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-center">
                                        <div class="flex flex-col sm:flex-row gap-1 sm:gap-2 justify-center">
                                            <a href="<?php echo e(route('admin.schedules.edit', [$class, $schedule])); ?>" class="inline-flex items-center justify-center gap-1 bg-blue-500 text-white px-2 sm:px-3 py-1 rounded text-xs font-semibold hover:bg-blue-600 transition whitespace-nowrap">
                                                <i class="fas fa-edit"></i> <span class="hidden sm:inline">Edit</span><span class="sm:hidden">Ed</span>
                                            </a>
                                            <form method="POST" action="<?php echo e(route('admin.schedules.destroy', [$class, $schedule])); ?>" class="inline delete-form">
                                                <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                                <button type="button" onclick="confirmDelete(event, '<?php echo e($schedule->day); ?>')" class="inline-flex items-center justify-center gap-1 bg-red-600 text-white px-2 sm:px-3 py-1 rounded text-xs font-semibold hover:bg-red-700 transition whitespace-nowrap w-full sm:w-auto">
                                                    <i class="fas fa-trash"></i> <span class="hidden sm:inline">Hapus</span><span class="sm:hidden">Del</span>
                                                </button>
                                            </form>
                                        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div class="bg-gradient-to-r from-red-500 to-red-600 px-3 sm:px-6 py-3 sm:py-4 text-white flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
            <h3 class="text-base sm:text-lg font-bold flex items-center gap-2 m-0 flex-wrap">
                ...
                <span class="bg-red-700 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm"><?php echo e($class->students->count()); ?></span>
            </h3>
            ...
            <?php else: ?>
                <div class="overflow-x-auto">
                    <table class="w-full text-xs sm:text-sm">
                        ...
                        <tbody>
                            <?php $__currentLoopData = $class->students; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i => $student): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="border-b border-gray-200 hover:bg-gray-50 transition">
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-gray-600"><?php echo e($i + 1); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 font-semibold text-gray-900 truncate"><?php echo e($student->name); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-gray-600 hidden sm:table-cell truncate text-xs"><?php echo e($student->email); ?></td>
                                    <td class="px-3 sm:px-6 py-3 sm:py-4 text-center">
                                        <form method="POST"
                                              action="<?php echo e(route('admin.classes.students.destroy', [$class, $student])); ?>"
                                              style="display: inline;" class="delete-form">
                                            <?php echo csrf_field(); ?> <?php echo method_field('DELETE'); ?>
                                            <button type="button" onclick="confirmDelete(event, '<?php echo e($student->name); ?>')" class="inline-flex items-center justify-center gap-2 bg-red-600 text-white px-2 sm:px-4 py-2 rounded-lg font-semibold text-xs sm:text-sm hover:bg-red-700 transition whitespace-nowrap">
                                                <i class="fas fa-trash"></i> <span class="hidden sm:inline">Hapus</span><span class="sm:hidden">Del</span>
                                            </button>
                                        </form>
                                    </td>
    ...
    <div class="bg-white rounded-lg max-w-2xl w-full max-h-96 flex flex-col overflow-hidden">
        ...
        <div class="bg-gray-50 px-3 sm:px-6 py-3 sm:py-4 border-b border-gray-200 flex justify-between items-center">
            <h2 class="text-base sm:text-lg font-bold text-gray-900 flex items-center gap-2 m-0 truncate">
                <i class="fas fa-user-plus"></i> <span class="hidden sm:inline">Tambah Siswa ke <?php echo e($class->name); ?></span><span class="sm:hidden">Tambah Siswa</span>
            </h2>
        ...
        <!-- Modal Body -->
        <form method="POST" action="<?php echo e(route('admin.classes.students.store', $class)); ?>" id="addStudentForm" class="flex-1 overflow-y-auto p-3 sm:p-6">
            <?php echo csrf_field(); ?>
            ...
            <?php else: ?>
                <div class="space-y-3 sm:space-y-4">
                    ...
                    <div class="bg-blue-50 border-l-4 border-blue-500 p-2 sm:p-3 rounded text-xs sm:text-sm text-blue-800">
                        ...
                        <strong><?php echo e($availableStudents->count()); ?></strong> siswa
                    </div>
                </div>
            <?php endif; ?>
            ...
            <div class="flex flex-col sm:flex-row gap-2 sm:gap-3 pt-4 sm:pt-6 border-t border-gray-200 mt-4 sm:mt-6">
                ...
                <?php if($availableStudents->isNotEmpty()): ?>
...
<script>
const StudentModal = (() => {
    const STUDENTS = <?php echo json_encode($availableStudents->map(fn($s) => ['id'=>$s->id, 'name'=>$s->name, 'email'=>$s->email])->values()->toArray()) ?>;
    const PER_PAGE = 8;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\xampp\htdocs\Project Akhir\elearning-srma\resources\views/admin/classes/show.blade.php ENDPATH**/ ?>