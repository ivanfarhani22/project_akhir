    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        <?php $__currentLoopData = $classSubjects; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cs): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            ...
            <div class="group bg-white rounded-2xl border-2 border-gray-100 hover:border-[#A41E35] hover:shadow-lg transition-all duration-200 overflow-hidden flex flex-col cursor-pointer hover:-translate-y-0.5"
                 role="link"
                 tabindex="0"
                 onclick="window.location.href='<?php echo e($detailUrl); ?>'"
                 onkeydown="if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); window.location.href='<?php echo e($detailUrl); ?>'; }">
                ...
                <div class="p-5 flex flex-col flex-1">
                    <div class="flex justify-between items-start gap-3 mb-3">
                        ...
                        <span class="inline-flex items-center gap-1 bg-blue-50 text-blue-600 border border-blue-100 text-xs font-semibold px-2.5 py-1 rounded-full whitespace-nowrap flex-shrink-0">
                            <i class="fas fa-users text-[10px]"></i> <?php echo e($cs->eClass->students->count()); ?>

                        </span>
                    </div>

                    <?php if($cs->eClass->description): ?>
                        <p class="text-xs text-gray-500 leading-relaxed mb-4"><?php echo e(Str::limit($cs->eClass->description, 90)); ?></p>
                    <?php endif; ?>

                    <div class="grid grid-cols-2 gap-3 mt-auto mb-4">
                        <div class="text-center bg-gray-50 border border-gray-100 rounded-xl py-3">
                            <p class="text-2xl font-extrabold text-[#A41E35]"><?php echo e($materialsCount); ?></p>
                            <p class="text-xs text-gray-400 font-medium mt-0.5"><i class="fas fa-book mr-1"></i>Materi</p>
                        </div>
                        <div class="text-center bg-gray-50 border border-gray-100 rounded-xl py-3">
                            <p class="text-2xl font-extrabold text-blue-600"><?php echo e($assignmentsCount); ?></p>
                            <p class="text-xs text-gray-400 font-medium mt-0.5"><i class="fas fa-tasks mr-1"></i>Tugas</p>
                        </div>
                    </div>

                    <div class="flex gap-2" onclick="event.stopPropagation()">
                        <a href="<?php echo e(route('guru.materials.index', ['class_id' => $cs->eClass->id, 'class_subject_id' => $cs->id])); ?>"
                        ...
                        <a href="<?php echo e(route('guru.assignments.index', ['class_id' => $cs->eClass->id, 'class_subject_id' => $cs->id])); ?>"
                           onclick="event.stopPropagation()"
...
<?php echo $__env->make('layouts.guru', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\xampp\htdocs\Project Akhir\elearning-srma\resources\views/guru/classes/index.blade.php ENDPATH**/ ?>