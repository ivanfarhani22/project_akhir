...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <form action="{{ route('admin.student-distribution.update', $studentDistribution->id) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <div class="w-full px-4 py-2 bg-gray-100 border border-gray-300 rounded-lg text-gray-700">
                    {{ $studentDistribution->academic_year }}
                </div>
            ...
            <div>
                ...
                <input type="text" name="district" value="{{ old('district', $studentDistribution->district) }}" required placeholder="contoh: Kecamatan Nglegok"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent @error('district') border-red-500 @enderror">
                @error('district')
            ...
            <div>
                ...
                <input type="number" name="student_count" value="{{ old('student_count', $studentDistribution->student_count) }}" min="0" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent @error('student_count') border-red-500 @enderror">
                @error('student_count')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
