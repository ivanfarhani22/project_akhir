...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <form action="{{ route('admin.student-data.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                @php
                    ...
                    $academicYears = array_reverse($academicYears); // Urutkan dari terbaru
                @endphp
            ...
            <div>
                ...
                <select name="class_name" required
                    ...
                    <option value="X" {{ old('class_name') === 'X' ? 'selected' : '' }}>Kelas X</option>
                    <option value="XI" {{ old('class_name') === 'XI' ? 'selected' : '' }}>Kelas XI</option>
                    <option value="XII" {{ old('class_name') === 'XII' ? 'selected' : '' }}>Kelas XII</option>
                </select>
                @error('class_name')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="male_count" value="{{ old('male_count', 0) }}" min="0" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    @error('male_count')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="number" name="female_count" value="{{ old('female_count', 0) }}" min="0" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    @error('female_count')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            <div>
                ...
                <input type="number" name="study_groups" value="{{ old('study_groups', 1) }}" min="1" required
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                @error('study_groups')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
