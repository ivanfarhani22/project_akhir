...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-sm p-4">
        <form method="GET" action="{{ route('admin.student-data.index') }}" class="flex flex-col sm:flex-row items-start sm:items-center gap-3">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-blue-50 rounded-xl p-4">
            <p class="text-sm text-blue-600 font-medium">Total Laki-laki{{ $selectedYear === 'all' ? ' (Keseluruhan)' : '' }}</p>
            <p class="text-2xl font-bold text-blue-700">{{ number_format($stats['total_male']) }}</p>
        </div>
        <div class="bg-pink-50 rounded-xl p-4">
            <p class="text-sm text-pink-600 font-medium">Total Perempuan{{ $selectedYear === 'all' ? ' (Keseluruhan)' : '' }}</p>
            <p class="text-2xl font-bold text-pink-700">{{ number_format($stats['total_female']) }}</p>
        </div>
        ...
        <div class="bg-purple-50 rounded-xl p-4">
            <p class="text-sm text-purple-600 font-medium">Total Rombel{{ $selectedYear === 'all' ? ' (Keseluruhan)' : '' }}</p>
            <p class="text-2xl font-bold text-purple-700">{{ number_format($stats['total_rombel']) }}</p>
        </div>
    </div>
    @endif
