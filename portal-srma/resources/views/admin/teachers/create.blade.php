...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-sm">
        ...
        <form action="{{ route('admin.teachers.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="position" value="{{ old('position') }}" placeholder="contoh: Kepala Sekolah, Guru"
                       class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                @error('position')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
